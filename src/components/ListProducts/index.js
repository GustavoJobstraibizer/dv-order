...
export default function ListProducts() {
  ...
  useEffect(() => {
    const getProductsByName = async () => {
      ...
      const res = await db.collection("products").where("name", "==", term).get();
  ...
  return (
    <>
      <Link className="btn btn-success mt-2" to="/produto">
        Novo
      </Link>
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product.id}>
              ...
              <td>
                <FontAwesomeIcon onClick={() => handleEdit(product)} className="text-info mr-2" icon={faEdit} />
              </td>
            </tr>
          ))}
        </tbody>
